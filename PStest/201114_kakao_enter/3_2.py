...
def segment(x, space):
    ...
    max_val = -1
    for i in range(0,len(space),x):
        if i + x - 1 >= len(space):
            val = query(space, tree_list, 1, 0, len(space) - 1, len(space) - x, len(space) - 1)
        else:
            val = query(space, tree_list, 1, 0, len(space) - 1, i, i + x - 1)
        if val > max_val :
            max_val = val
...
test_cases = [
    ...
    [2, [3,1,1,1], 1],
    [3, [2,5,4,6,8], 4],
    [1, [i for i in range(1000000)], -1],
    [35, [i for i in range(1000000)], -1],
    [805, [i for i in range(1000000)], -1],
    [561950, [i for i in range(1000000)], -1]
]
