...
def search():
    ...
    ## 데이터 입력 ##
    for _ in range(n):
        emp = list(map(int, f.readline().strip().split(' ')))
        emp_id = emp[0]
        boss_id = emp[1]
        res_num = emp[2]
    ...
    ## 쿼리 처리 ##
    for _ in range(q):
        query = list(map(int, f.readline().strip().split(' ')))

        emp_id = query[0]
        res_id = query[1]

        find_res(emp_id)
        print(find_emp_res(emp_id,res_id))
...
if __name__ == '__main__':
    file_path = os.getcwd() + '\\test_cases_2\\input001.txt'
    f = open(file_path, 'r')
